...
# ggsave('../../5_Plots/3_Omic_measurements/pysr/Prediction_spread_all.png', p, width = 7, height = 4)
...
plot_filename <- '../../5_Plots/3_Omic_measurements/Prediction_vs_actual_all.png'
...
p_bar_two <- ggplot(test_df, aes(x = test_df$rownames, y = Dif, fill = Group)) +
  ...
  guides(fill = guide_legend(nrow = 3, byrow = TRUE))  # Adjust legend to have three rows
# Save the plot
plot_filename_scatter <- '../../5_Plots/3_Omic_measurements/Actual_vs_Predicted_difference.png'
...
p <- ggplot(dat_all, aes(x = Iteration, y = log(RMSE), color = Complexity)) +
  ...
  # Save
  plot_filename <- '../../5_Plots/3_Omic_measurements/pysr/sensitivity.png'
...
p <- ggplot(dat_full, aes(x = reorder(Feature, -Importance), y = Importance)) +
  ...
  scale_y_continuous(expand = c(0, 0)) 
# Save
plot_filename <- '../../5_Plots/3_Omic_measurements/rf/variable_importance_all.png'
ggsave(plot_filename, p, width = 4, height = 4)
```
...
p <- ggplot(dat_full, aes(x = reorder(Feature, -Importance), y = Importance)) +
  ...
  scale_y_continuous(expand = c(0, 0)) 
# Save
plot_filename <- '../../5_Plots/3_Omic_measurements/xgb/variable_importance_all.png'
ggsave(plot_filename, p, width = 4, height = 4)
```
