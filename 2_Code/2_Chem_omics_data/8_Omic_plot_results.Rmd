...
train_y_pred = pd.read_pickle("3_Data_intermediates/3_Omic_measurements/Omic_training_predictions_pysr_DEG")
test_y_pred = pd.read_pickle("3_Data_intermediates/3_Omic_measurements/Omic_test_predictions_pysr_DEG")
...
train_y_pred_rf = pd.read_pickle("3_Data_intermediates/3_Omic_measurements/training_predictions_rf_DEG")
test_y_pred_rf = pd.read_pickle("3_Data_intermediates/3_Omic_measurements/test_predictions_rf_DEG")
...
p_bar_two <- ggplot(test_df, aes(x = test_df$rownames, y = Dif, fill = Group)) +
  ...
  guides(fill = guide_legend(nrow = 3, byrow = TRUE))  # Adjust legend to have three rows
# Save the plot
plot_filename_scatter <- paste0('C:/Users/jrchapp3/OneDrive - University of North Carolina at Chapel Hill/Symbolic_regression_github/NIH_Cloud_NOSI/5_Plots/3_Omic_measurements/Actual_vs_Predicted_difference.png')
...
p <- ggplot(dat_all, aes(x = Iteration, y = log(RMSE), color = Complexity)) +
  ...
  # Save
  plot_filename <- 'C:/Users/jrchapp3/OneDrive - University of North Carolina at Chapel Hill/Symbolic_regression_github/NIH_Cloud_NOSI/5_Plots/3_Omic_measurements/pysr/sensitivity.png'
...
genes_clean <- left_join(genes, degs_clean) %>%
  select(Gene_Symbol) %>%
  unique() %>%
  mutate(ifelse(Gene_Symbol == '3-Mar', 'March3', Gene_Symbol))
...
combined_dat <- combined_dat %>%
  ...
  mutate(Gene_Symbol = ifelse(Gene_Symbol == '3-Mar', 'March3', Gene_Symbol))
...
p <- ggplot(dat_full, aes(x = reorder(Feature, -Importance), y = Importance)) +
  ...
  scale_y_continuous(expand = c(0, 0)) 
# Save
plot_filename <- paste0('C:/Users/jrchapp3/OneDrive - University of North Carolina at Chapel Hill/Symbolic_regression_github/NIH_Cloud_NOSI/5_Plots/3_Omic_measurements/rf/variable_importance_all.png')
ggsave(plot_filename, p, width = 4, height = 4)
```
 ...
 # XGBoost importance
```{r}
dat <- read.csv('..\\..\\3_Data_intermediates\\3_Omic_measurements\\xgb_variable_importance_Elastic.csv')
...
dat_full <- dat_clean %>%
  arrange(-Importance) %>%
  .[1:15,] %>%
  mutate(Feature = ifelse(Feature== '3-Mar', 'March3', Feature))
...
p <- ggplot(dat_full, aes(x = reorder(Feature, -Importance), y = Importance)) +
  ...
  scale_y_continuous(expand = c(0, 0)) 
# Save
plot_filename <- paste0('C:/Users/jrchapp3/OneDrive - University of North Carolina at Chapel Hill/Symbolic_regression_github/NIH_Cloud_NOSI/5_Plots/3_Omic_measurements/xgb/variable_importance_all.png')
ggsave(plot_filename, p, width = 4, height = 4)
```
