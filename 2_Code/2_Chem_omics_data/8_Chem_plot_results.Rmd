...
test_combined <- data.frame(
  Predicted = test_preds_list[[3]][6, 'Predictions'][[1]], # Elastic predictions for 6th HOF equation
  Actual = test_y
)
...
print(p)
# Save
plot_filename <- paste0('C:/Users/jrchapp3/OneDrive - University of North Carolina at Chapel Hill/Symbolic_regression_github/NIH_Cloud_NOSI/5_Plots/2_Chemical_measurements/pysr/Prediction_spread_all.png')
...
plot_filename <- paste0('C:/Users/jrchapp3/OneDrive - University of North Carolina at Chapel Hill/Symbolic_regression_github/NIH_Cloud_NOSI/5_Plots/2_Chemical_measurements/pysr/Prediction_vs_actual_all.png')
...
p_bar_two <- ggplot(test_df, aes(x = test_df$rownames, y = Dif, fill = Group)) +
  ...
  scale_fill_manual(values = custom_colors)
# Save the plot
plot_filename_scatter <- paste0('C:/Users/jrchapp3/OneDrive - University of North Carolina at Chapel Hill/Symbolic_regression_github/NIH_Cloud_NOSI/5_Plots/2_Chemical_measurements/pysr/Actual_vs_Predicted_difference.png')
...
```{r}
# Load in data
sens_full <- read.csv('C:\\Users\\jrchapp3\\OneDrive - University of North Carolina at Chapel Hill\\Symbolic_regression_github\\NIH_Cloud_NOSI\\4_Model_results\\2_Chemical_measurements\\pysr\\pysr_RMSE_sensitivity_Full.csv')
sens_pca <- read.csv('C:\\Users\\jrchapp3\\OneDrive - University of North Carolina at Chapel Hill\\Symbolic_regression_github\\NIH_Cloud_NOSI\\4_Model_results\\2_Chemical_measurements\\pysr\\pysr_RMSE_sensitivity_PCA.csv')
sens_elastic <- sens_full <- read.csv('C:\\Users\\jrchapp3\\OneDrive - University of North Carolina at Chapel Hill\\Symbolic_regression_github\\NIH_Cloud_NOSI\\4_Model_results\\2_Chemical_measurements\\pysr\\pysr_RMSE_sensitivity_elastic.csv')
...
p <- ggplot(dat_all, aes(x = Iteration, y = log(RMSE), color = Complexity)) +
  ...
  # Save
  plot_filename <- 'C:/Users/jrchapp3/OneDrive - University of North Carolina at Chapel Hill/Symbolic_regression_github/NIH_Cloud_NOSI/5_Plots/2_Chemical_measurements/pysr/sensitivity.png'
  ggsave(plot_filename, p, width = 8, height = 4)
...
p <- ggplot(combined_dat, aes(x = reorder_within(Chemical, -var_importance, dataset), y = var_importance, fill = `Chemical Class`)) +
  ...
  scale_fill_manual(values = new_palette, 
                    breaks = c("Inorganics", "PAHs", "Ions", "Methoxyphenols")) + 
...
# Train-test RMSE
```{r}
df <- read.csv('..\\..\\3_Data_intermediates\\2_Chemical_measurements\\Chem_rmse_values_pysr_Elastic.csv')
...
p <- ggplot(df_long, aes(x = HOF.equation, y = RMSE, color = Type, group = Type)) +
  ...
  theme(
    axis.text.x = element_text(size = 8, angle = 0, hjust = 0.6),
    axis.title.x = element_text(size = 10, face = "bold"),
    axis.title.y = element_text(size = 10, face = "bold"),
    legend.title = element_blank(),
    legend.text = element_text(size = 8),
...
# RF importance
```{r}
dat_full <- read.csv('..\\..\\3_Data_intermediates\\2_Chemical_measurements\\rf_variable_importance_Full.csv')
dat_pca <- read.csv('..\\..\\3_Data_intermediates\\2_Chemical_measurements\\rf_variable_importance_PCA.csv')
dat_elastic <- read.csv('..\\..\\3_Data_intermediates\\2_Chemical_measurements\\rf_variable_importance_Elastic.csv')
...
# XGBoost importance
```{r}
dat_full <- read.csv('..\\..\\3_Data_intermediates\\2_Chemical_measurements\\xgb_variable_importance_Full.csv')
dat_pca <- read.csv('..\\..\\3_Data_intermediates\\2_Chemical_measurements\\xgb_variable_importance_PCA.csv')
dat_elastic <- read.csv('..\\..\\3_Data_intermediates\\2_Chemical_measurements\\xgb_variable_importance_Elastic.csv')
...
# Save
plot_filename <- paste0('C:/Users/Jessie PC/OneDrive - University of North Carolina at Chapel Hill/Symbolic_regression_github/NIH_Cloud_NOSI/5_Plots/2_Chemical_measurements/xgb/variable_importance_all.png')
ggsave(plot_filename, p, width = 4, height = 4)
```
