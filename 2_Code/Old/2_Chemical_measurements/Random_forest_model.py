...
for i in range(len(train_input_dict)):
    ...
    # Save predictions
    file_name = f'Data_inputs/2_Chemical_measurements/training_predictions_rf_{key}'
    pd.DataFrame(train_pred).to_pickle(file_name)
    file_name = f'Data_inputs/2_Chemical_measurements/test_predictions_rf_{key}'
    pd.DataFrame(test_pred).to_pickle(file_name)
