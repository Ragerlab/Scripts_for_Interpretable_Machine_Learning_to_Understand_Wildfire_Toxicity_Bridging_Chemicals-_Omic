...
# 1) Read all VI csvs
# ----------------------------
vi_dir <- "C:\\Users\\Jessie PC\\OneDrive - University of North Carolina at Chapel Hill\\Symbolic_regression_github\\NIH_Cloud_NOSI\\4_Model_results\\1_Simulated_data\\pysr\\Variable_importance"
...
# ✅ y-axis labels: ONLY X1, X2, ...
rownames(mat) <- heat_wide$chem
colnames(mat) <- as.character(1:ncol(mat))

# ---- NAs -> 0 (keep if you want, but we'll color zeros grey below) ----
mat[is.na(mat)] <- 0

# ✅ gap lines between High/Med/Low blocks (compute from the complexity column directly)
n_high <- sum(heat_wide$complexity == "high")
n_med  <- sum(heat_wide$complexity == "med")
gaps_row <- c(n_high, n_high + n_med)

# ✅ make exact zeros light grey (convert zeros to NA for plotting)
mat_plot <- mat
...
p <- pheatmap(
  ...
  main = "Variable DECS",
  angle_col = 0,
  na_col = "grey90"   # ✅ light grey for zeros (now NA)
)
