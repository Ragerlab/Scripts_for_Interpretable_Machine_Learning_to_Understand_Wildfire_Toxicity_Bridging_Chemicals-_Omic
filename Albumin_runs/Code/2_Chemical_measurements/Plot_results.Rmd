...
train_y_pred_lasso = pd.read_pickle("Data_inputs/2_Chemical_measurements/training_predictions_Elastic")
test_y_pred_lasso = pd.read_pickle("Data_inputs/2_Chemical_measurements/test_predictions_Elastic")
train_y_pred_albumin = pd.read_pickle("Data_inputs/2_Chemical_measurements/training_predictions_Albumin")
test_y_pred_albumin = pd.read_pickle("Data_inputs/2_Chemical_measurements/test_predictions_Albumin")

# Store the inputs (Full, PCA, Elastic) in a list
input_names = ['Full', 'PCA', 'Elastic', 'Albumin']
train_preds_list = [train_y_pred, train_y_pred_pca, train_y_pred_lasso, train_y_pred_albumin]
test_preds_list = [test_y_pred, test_y_pred_pca, test_y_pred_lasso, test_y_pred_albumin]
```
...
train_y <- py$train_y
test_y <- py$test_y
test_y_albumin <- py$test_y_albumin
...
combined_test_dat <- do.call(rbind, lapply(seq_along(input_names), function(i) {
  ...
  if(input_name == 'Albumin'){
    test_combined <- data.frame(
      Actual = test_y_albumin,
      Predicted = test_y_pred[,1]
    )
  }
...
# Define levels
combined_test_dat$input_name <- factor(combined_test_dat$input_name, levels = c('Full', 'PCA', 'Elastic', 'Albumin'))
...
print(p)
# Save
plot_filename <- paste0('C:/Users/jrchapp3/OneDrive - University of North Carolina at Chapel Hill/Symbolic_regression_github/NIH_Cloud_NOSI/images/2_Chemical_measurements/pysr/Prediction_spread_all.png')
...
p_bar <- ggplot(combined_test_dat, aes(x = rownames(combined_test_dat), y = Actual, fill = Group)) +
  ...
  facet_wrap(~ input_name, scales = "free_x") +
  guides(fill = guide_legend(nrow = 3, byrow = TRUE))  # Adjust legend to have three rows
plot_filename <- paste0('C:/Users/jrchapp3/OneDrive - University of North Carolina at Chapel Hill/Symbolic_regression_github/NIH_Cloud_NOSI/images/2_Chemical_measurements/pysr/Prediction_vs_actual_all.png')
